...
/** ms between missile shots (client UX + server authority). */
...
/**
 * Vibrant, saturated tints (0xRRGGBB) for missiles and splatters.
...
export interface missileFiredEvent {
  ...
  dz: number;
  speed: number;
  /** Range multiplier applied to globeRadius * missile_RANGE_FACTOR (clamped). Optional for backward compat. */
  rangeMult?: number;
}

/** Client pushes its missile upgrade flags so the server mirrors them on hit test + cooldown. */
export interface missileUpgradeFlags {
  doubleTap: boolean;
  speedMult: number;
  rangeMult: number;
}
...
export interface ServerToClientEvents {
  ...
  "missile:fired": (event: missileFiredEvent) => void;
  "missile:hit": (event: missileHitEvent) => void;
...
export interface ClientToServerEvents {
  ...
  "missile:fire": () => void;
  "missile:setUpgrades": (flags: missileUpgradeFlags) => void;
