...
docker run -d --name DroneWars-db -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=DroneWars -p 5432:5432 postgres:16
...
Production URLs:
- **Client**: https://drone-wars.vercel.app (Vercel)
...
npm i -g @railway/cli
railway login
railway link    # select the DroneWars-server project
```
...
```bash
npx vercel login
npx vercel link   # select the drone-wars project
```
...
```
VITE_SERVER_URL=https://DroneWars-api-production.up.railway.app
```
...
| `CLIENT_URL` | `https://drone-wars.vercel.app` (for CORS; comma-separated for extras) |
