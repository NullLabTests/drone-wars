...
import type { PlayerState, Vehicle } from "@DroneWars/shared";
import {
  slerpPlayerState,
  deadReckon,
  buildPlaneMatrix,
...
class RemotePlane {
  ...
  private applyToMesh(state: PartialState, dt: number) {
    ...
    if (this.vehicle === "boat") {
      ...
      const bank = (state.bankAngle ?? 0) + this.missileWobbleBank;
      m = buildPlaneMatrix(
        qPos,
        state.heading,
        state.pitch,
...
export class RemotePlaneManager {
  ...
  triggermissileHitWobble(playerId: string) {
    this.planes.get(playerId)?.triggermissileHitWobble();
  }
