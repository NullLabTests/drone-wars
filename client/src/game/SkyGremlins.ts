...
import { missile_COLOR_PALETTE } from "@DroneWars/shared";
import type { Plane } from "./Plane";
import type { missileSystem, ProjectileStepInfo } from "./missileSystem";
import {
...
type GremlinState = {
  ...
  /** >0: prefer long-range shots and wide standoff after player missile damage. */
...
export class SkyGremlins {
  ...
  constructor(
    ...
    /** Called on every local missile hit that damages a gremlin; `isKill` is true on the killing shot. */
    private readonly onGremlinmissileHit?: (isKing: boolean, isKill: boolean) => void,
    /** When a gremlin missile hits the local player (after slow / splatter). */
    ...
    this.removeProjectileStepListener = this.missileSystem.addProjectileStepListener((info) => {
  ...
  setSuspended(suspended: boolean) {
    ...
    if (suspended) {
      this.missileSystem.clearProjectilesByShooterPrefix(GREMLIN_SHOOTER_PREFIX);
    }
  }
    ...
    return {
      ...
      paintColor: king
        ...
        : missile_COLOR_PALETTE[
            Math.floor(random() * missile_COLOR_PALETTE.length)
          ]!,
    ...
    if (
      ...
      if (this.forwardScratch.dot(this.directionScratch) >= fireDot) {
        ...
        if (gremlin.aimTimer >= 0.45) {
          ...
          this.missileSystem.spawnLocalProjectile({
  ...
  private handleProjectileStep(info: ProjectileStepInfo) {
    ...
    if (info.shooterId.startsWith(GREMLIN_SHOOTER_PREFIX)) {
      ...
      info.consume();
      this.missileSystem.triggerLocalPlayerHit(
    ...
    for (const gremlin of targets) {
      ...
      info.consume();
      this.missileSystem.playImpactAtGroup(gremlin.root, info.color, false);

      gremlin.health--;
      const isKill = gremlin.health <= 0;
      this.onGremlinmissileHit?.(gremlin.isKing === true, isKill);
      if (isKill) {
