...
import type { Vehicle } from "@DroneWars/shared";
import {
  buildPlaneMatrix,
...
/** HP from sky-gremlin missiles (matches gremlin bar pill geometry). Base 10 + 30%. */
const PL_HP_MAX = 14;
...
export class Plane {
  ...
  /** Stronger slow from the Gremlin King's missiles (does not stack with gremlin slow — king takes priority). */
  ...
  /** Damped sin — missile hit rolls the mesh left/right briefly (visual only). */
  private missileWobbleAmp = 0;
  private missileWobblePhase = 0;
  private missileWobbleBank = 0;
  ...
  update(
    ...
    if (this.missileWobbleAmp > 0.002) {
      this.missileWobblePhase += dt * 19;
      this.missileWobbleBank =
        Math.sin(this.missileWobblePhase) * this.missileWobbleAmp;
      this.missileWobbleAmp *= Math.exp(-4.2 * dt);
    } else {
      this.missileWobbleAmp = 0;
      this.missileWobbleBank = 0;
    }

    this.applyMatrix();
  }

  /** Called when this plane is struck by a missile (local client). */
  triggermissileHitWobble() {
    this.missileWobbleAmp = 0.42;
    this.missileWobblePhase = 0;
  }
  ...
  /**
   * Gremlin missile damage. Returns true the first time health reaches 0.
   * @param isKing - king shots deal more damage
   */
  applyGremlinmissileDamage(isKing: boolean): boolean {
  ...
  applyMatrix() {
    const m = buildPlaneMatrix(
      ...
      this.heading,
      this.pitch,
      this.bankAngle + this.missileWobbleBank + this.rollAngle,
      this.altitude,
      this.globeRadius,
    );
