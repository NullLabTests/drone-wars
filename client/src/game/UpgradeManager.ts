...
export interface UpgradeState {
  ...
  /** Max gremlin missile HP = round(PL_HP_MAX * this). */
  ...
  missileRangeMult: number;
  /** Double-Tap burst fire: 2 missiles ~90ms apart, longer post-burst cooldown. */
  missileDoubleTapEnabled: boolean;
...
const PLANE_UPGRADES: UpgradeDefinition[] = [
  ...
  },
  {
    id: "sharpshooter",
    name: "Sharpshooter",
    description: "missile speed +30%, range +25%",
    category: "performance",
    apply: (s) => {
      s.missileSpeedMult *= 1.30;
      s.missileRangeMult *= 1.25;
    },
  },
  {
    id: "double_tap",
    name: "Double Tap",
    description: "Fire two missiles in rapid succession; slower post-burst cooldown",
    category: "performance",
    apply: (s) => { s.missileDoubleTapEnabled = true; },
  },
  {
    id: "long_shot",
    name: "Long Shot",
    description: "+20% biplane missile range",
    category: "performance",
    apply: (s) => { s.missileRangeMult *= 1.2; },
  },
  {
    id: "hull_reinforced",
