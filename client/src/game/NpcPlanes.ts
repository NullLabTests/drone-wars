...
class NpcPlane {
  ...
  /** Tilt-wobble on missile hit — same feel as RemotePlane. */
  triggerHitWobble() {
    this.wobbleAmp = 0.42;
    this.wobblePhase = 0;
  }
...
export class NpcPlanes {
   ...
   * - missileSystem: optional — registers a hit listener so NPC groups receive decals + wobble
   */
  update(
    dt: number,
  ...
  /**
   * Register a projectile step listener so NPCs receive missile decals + tilt-wobble.
   * Returns an unsubscribe handle — call it in dispose().
   */
  registermissileListener(missileSystem: missileSystem): () => void {
    const npcGroups = this.planes.map((n) => ({ group: n.group, npc: n }));
    const _segDir = new Vector3();
    const _toCenter = new Vector3();
    const HIT_R_SQ = 0.22 * 0.22;

    return missileSystem.addProjectileStepListener((step) => {
      ...
      for (const { group, npc } of npcGroups) {
        ...
        if (closest.distanceToSquared(center) < HIT_R_SQ) {
          ...
          missileSystem.playImpactAtGroup(group, step.color, true, splatSeed, step.splatterScale ?? 1);
