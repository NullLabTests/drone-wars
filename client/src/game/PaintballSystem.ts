...
import {
  missile_BURST_WINDOW_MS,
  missile_COLOR_PALETTE,
  missile_COOLDOWN_MS,
  missile_RANGE_FACTOR,
  missile_SPEED,
  SPLATTER_LIFETIME_SEC,
  type missileFiredEvent,
  type missileHitEvent,
} from "@DroneWars/shared";

/** Delay between the two shots of a Double Tap burst (ms). */
const DOUBLE_TAP_INTERVAL_MS = 90;
import { missileRayFromPlaneState } from "./SphericalMath";
import type { Plane } from "./Plane";
import type { RemotePlaneManager } from "./RemotePlane";
import { missileSplashPool } from "./missileSplash";
...
function createmissileMaterial(colorHex: number): ShaderMaterial {
...
/**
 * missile projectiles, local cooldown, splatter decals, socket hooks.
 */
export class missileSystem {
  ...
  private readonly splashPool = new missileSplashPool();
  ...
  constructor(
    ...
    private onLocalPlayermissileHit?: (
      colorHex?: number,
      ctx?: { fromGremlin?: boolean; gremlinKing?: boolean },
    ) => void,
    private onmissileVictimWobble?: (victimId: string) => void,
    /** One-shot when a projectile actually spawns (local + remote). */
    private onmissileShoot?: () => void,
    /** One-shot when a missile hits a plane. `distant` = true when our shot hit someone else. */
    private onmissileImpact?: (splatSeed: number, distant: boolean) => void,
  ...
  spawnLocalProjectile(options: {
    ...
    const color =
      options.color ??
      missile_COLOR_PALETTE[
        Math.floor(Math.random() * missile_COLOR_PALETTE.length)
      ]!;
    const didSpawn = this.spawnProjectile({
      ...
      speed: options.speed ?? missile_SPEED,
  ...
  setLocalmissileMultipliers(speedMult: number, rangeMult: number) {
  ...
  tryLocalFire(plane: Plane) {
    ...
    if (this.localDoubleTapEnabled) {
      // Burst mode: one emission per missile_BURST_WINDOW_MS (gates on the first shot of the burst).
      if (now - this.lastBurstStart < missile_BURST_WINDOW_MS) return;
    ...
    if (now - this.lastLocalFire < missile_COOLDOWN_MS) return;
    this.lastLocalFire = now;
    this.fireOnce(plane);
  }

  private fireOnce(plane: Plane) {
    const ray = missileRayFromPlaneState(
      plane.qPosition,
      plane.heading,
      plane.pitch,
      plane.altitude,
    ...
    const shot = {
      ...
      dy: ray.direction.y,
      dz: ray.direction.z,
      speed: missile_SPEED * this.localSpeedMult,
      rangeMult: this.localRangeMult,
    };

    const sock = this.getSocket();
    if (sock?.connected) {
      sock.emitmissileFire();
      /** Optimistic spawn so the shot appears even if `missile:fired` is slow or lost (prod WS / CDN). */
      if (this.spawnProjectile(shot)) this.onmissileShoot?.();
      return;
    }
  ...
  onmissileFired(ev: missileFiredEvent) {
  ...
  private spawnProjectile(ev: {
    ...
    dz: number;
    speed: number;
    /** Optional Sharpshooter range multiplier. Remote echoes carry this via `missileFiredEvent`. */
    rangeMult?: number;
    ballRadius?: number;
    splatterScale?: number;
  }): boolean {
    ...
    const maxRange = this.globeRadius * missile_RANGE_FACTOR * rangeMult;
    ...
    const mat = createmissileMaterial(ev.color);
  ...
  onmissileHit(ev: missileHitEvent, localPlaneGroup: Group | null) {
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
    ...
    if (ev.victimId === myId) {
      this.onLocalPlayermissileHit?.(ev.color, undefined);
    }
    this.onmissileVictimWobble?.(ev.victimId);
    this.applyImpactAtGroup(victimRoot, ev.color, ev.splatSeed);
    if (ev.victimId === myId) {
      this.onmissileImpact?.(ev.splatSeed, false);
    } else if (ev.shooterId === myId) {
      this.onmissileImpact?.(ev.splatSeed, true);
    }
  }
  ...
  playImpactAtGroup(
    ...
    this.onmissileImpact?.(splatSeed, distant);
  }
  ...
  playMothSparkAtWorld(worldPos: Vector3, distant = false) {
    ...
    this.onmissileImpact?.(seed, distant);
  }
          ...
          if (closest.distanceToSquared(_bPos) < BALLOON_HIT_R_SQ) {
            ...
            if (balloonInner) {
              ...
              if (p.shooterId === myId) {
                ...
                this.onmissileImpact?.(splatSeed, true);
              }
            }
            hitBalloon = true;
            break;
