...
/** Keeps chatter from firing on every missile hit. */
...
export class Game {
  ...
  private npcmissileUnsub: (() => void) | null = null;
  ...
  private onLocalPlayerGremlinmissileHit(isKing: boolean) {
    if (!(this.localPlayer instanceof Plane)) return;
    if (this.localPlayer.applyGremlinmissileDamage(isKing)) {
  ...
  private startGame(vehicle: Vehicle) {
    ...
    this.missileSystem = new missileSystem(
      ...
      (colorHex?: number, ctx?: { fromGremlin?: boolean; gremlinKing?: boolean }) => {
        ...
        this.hud.showmissileSplatter(colorHex);
      },
      (victimId) => {
        const myId = this.socketClient?.id ?? "local";
        if (victimId === myId && this.localPlayer instanceof Plane) {
          this.localPlayer.triggermissileHitWobble();
          return;
        }
        this.remotePlanes.triggermissileHitWobble(victimId);
      },
      () => {
        this.audioManager.resumeContextIfNeeded();
    ...
    if (vehicle === "plane" && this.missileSystem) {
      this.skyGremlins = new SkyGremlins(
        ...
        (isKing) => {
          this.onLocalPlayerGremlinmissileHit(isKing);
        },
      );
    ...
    if (vehicle === "plane" || vehicle === "carpet") {
      ...
      if (this.missileSystem) {
        this.npcmissileUnsub = this.npcPlanes.registermissileListener(this.missileSystem);
      }
    }
  ...
  private teardownGameplaySession(reason = "session_ended") {
    ...
    this.npcmissileUnsub?.();
    this.npcmissileUnsub = null;
  ...
  private initNetworking(slug: string) {
    ...
    this.socketClient.onmissileFired((ev) => {
      this.missileSystem?.onmissileFired(ev);
    });

    this.socketClient.onmissileHit((ev) => {
      this.missileSystem?.onmissileHit(
        ev,
  ...
  private tick = () => {
    ...
    let { turnRate, forward, brake, elevate, descend, missile, specialAction, interact } =
    ...
    this.updateVehicleTutorial({ turnRate, forward, brake, elevate, missile, specialAction });
    ...
    this.localPlayer.visibility = 1;
    this.localPlayer.update(dt, turnRate, forward, brake, elevate, missile, descend);
    this.localPlayer.group.updateMatrixWorld(true);
    if (
      this.raceManager &&
      this.gamePhase === "flying" &&
    ...
    if (missile && this.localPlayer instanceof Plane && this.missileSystem) {
      this.missileSystem.tryLocalFire(this.localPlayer);
    }
    if (
      this.inCosmicVoid &&
      this.localPlayer instanceof Carpet &&
    ...
    this.missileSystem?.update(dt, this.cameraRig.camera.position);
  ...
  private propagateUpgrades() {
    ...
    if (this.missileSystem) {
      this.missileSystem.setLocalmissileMultipliers(
        s.missileSpeedMult,
        s.missileRangeMult,
      );
      this.missileSystem.setLocalDoubleTap(s.missileDoubleTapEnabled);
      this.socketClient?.emitmissileSetUpgrades({
        doubleTap: s.missileDoubleTapEnabled,
        speedMult: s.missileSpeedMult,
        rangeMult: s.missileRangeMult,
      });
    }
