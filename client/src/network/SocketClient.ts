...
export class SocketClient {
  ...
  emitmissileFire() {
    this.socket.emit("missile:fire");
  }

  emitmissileSetUpgrades(flags: missileUpgradeFlags) {
    this.socket.emit("missile:setUpgrades", flags);
  }
  ...
  onmissileFired(cb: (ev: missileFiredEvent) => void) {
    this.socket.on("missile:fired", cb);
  }

  onmissileHit(cb: (ev: missileHitEvent) => void) {
    this.socket.on("missile:hit", cb);
  }
