...
import {
  ...
  missile_COOLDOWN_MS,
  missile_RANGE_MULT_MAX,
  missile_SPEED_MULT_MAX,
} from "../missile/constants.js";
import { cartesianFromSpherical, computemissileShot } from "../missile/hitTest.js";
import { surfaceDisplacementAt } from "../terrain/TerrainSurface.js";

const REF_UP = new Vector3(0, 1, 0);

interface missileUpgradeRecord {
  doubleTap: boolean;
  speedMult: number;
  rangeMult: number;
}
...
export class Room {
  ...
  /** World globe radius — used for missile raycast (matches Prisma world row). */
  ...
  /** Rolling pair of the two most recent missile shot timestamps per socket (ms). */
  private missileShotHistory = new Map<string, number[]>();
  /** Client-reported missile upgrade flags per socket (server validates / clamps). */
  private missileUpgrades = new Map<string, missileUpgradeRecord>();
  ...
  removePlayer(socketId: string) {
    ...
    this.missileShotHistory.delete(socketId);
    this.missileUpgrades.delete(socketId);
    this.flagSuppressedPlayers.delete(socketId);
  ...
  /** Client pushes its missile upgrade flags so the hit test / cooldown mirror them. */
  setmissileUpgrades(socketId: string, flags: missileUpgradeFlags) {
    if (!this.players.has(socketId)) return;
    if (!flags || typeof flags !== "object") return;
    const record: missileUpgradeRecord = {
      doubleTap: flags.doubleTap === true,
      speedMult: clamp(Number(flags.speedMult) || 1, 1, missile_SPEED_MULT_MAX),
      rangeMult: clamp(Number(flags.rangeMult) || 1, 1, missile_RANGE_MULT_MAX),
    };
    this.missileUpgrades.set(socketId, record);
  }

  /** Server-authoritative missile: validates cooldown and vehicle; broadcasts to room. */
  firemissile(socketId: string) {
    ...
    const now = Date.now();
    const upgrades = this.missileUpgrades.get(socketId);
    ...
    const history = this.missileShotHistory.get(socketId) ?? [];
    const last1 = history[history.length - 1] ?? 0;
    const last2 = history[history.length - 2] ?? 0;

    if (doubleTap) {
      // Burst allowed: up to 2 shots per missile_BURST_WINDOW_MS; no sub-cooldown between shot 1 and shot 2.
      if (last2 > 0 && now - last2 < missile_BURST_WINDOW_MS) return;
    } else {
      // Classic single-shot cooldown.
      if (now - last1 < missile_COOLDOWN_MS) return;
    }

    history.push(now);
    if (history.length > 2) history.splice(0, history.length - 2);
    this.missileShotHistory.set(socketId, history);

    const others = Array.from(this.players.entries())
      .filter(([id]) => id !== socketId)
      .map(([id, p]) => ({ id, state: p.state }));

    const result = computemissileShot(me.state, socketId, others, this.globeRadius, {
      speedMult,
      rangeMult,
    });

    const firedPayload: missileFiredEvent = {
      shooterId: socketId,
      ...result.fired,
    };
    for (const [, p] of this.players) {
      p.socket.emit("missile:fired", firedPayload);
    }

    if (result.hit) {
      const hitPayload: missileHitEvent = {
        shooterId: socketId,
      ...
      for (const [, p] of this.players) {
        p.socket.emit("missile:hit", hitPayload);
      }
    }
  }
