...
import type { PlayerState } from "@DroneWars/shared";
import {
...
export interface missileShotResult {
  fired: {
    ox: number;
    oy: number;
    oz: number;
...
export interface missileShotParams {
 ...
 */
export function computemissileShot(
  shooter: PlayerState,
  shooterId: string,
  others: { id: string; state: PlayerState }[],
  globeRadius: number,
  params: missileShotParams = { speedMult: 1, rangeMult: 1 },
): missileShotResult {
  ...
  const maxRange = globeRadius * missile_RANGE_FACTOR * params.rangeMult;
  const shotSpeed = missile_SPEED * params.speedMult;

  const shotColor =
    missile_COLOR_PALETTE[Math.floor(Math.random() * missile_COLOR_PALETTE.length)]!;
